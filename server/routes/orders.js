...
function computeTotal(items, serviceType, pricing, deliveryFee, withDelivery) {
	...
	for (const [key, value] of Object.entries(items || {})) {
		if (key === 'autres') {
			...
			// Par défaut, pas de prix pour "autres"; à ajuster manuellement côté admin si besoin
			total += 0 * v;
			continue;
		}
...
module.exports = (state, { requireAdmin } = {}) => {
	...
	router.post('/', async (req, res) => {
		...
		if (!firstName || !lastName || !phone || !serviceType) return res.status(400).json({ error: 'Champs requis manquants' });
		...
		if (set) for (const client of set) client.write(`event: update\ndata: ${JSON.stringify({ type: 'created', order })}\n\n`);
		// web push
		try {
			const subs = await Subscription.find({ phone: order.phone }).lean();
			await Promise.all(
				subs.map((s) => webpush.sendNotification(s.subscription, JSON.stringify({ title: 'Commande bien reçue', body: `Merci ${order.firstName}!`, data: { id: order._id } })).catch(() => null))
			);
		} catch {}
		// FCM push
		try {
			...
			if (tokens.length) {
				await admin.messaging().sendEachForMulticast({ tokens: tokens.map(t => t.token), notification: { title: 'Commande bien reçue', body: `Merci ${order.firstName}!` }, data: { id: String(order._id) } });
			}
		} catch {}
	});
	...
	router.put('/:id', requireAdmin, async (req, res) => {
		...
		if (update.items || update.serviceType || typeof update.withDelivery === 'boolean') {
			...
			const withDelivery = typeof update.withDelivery === 'boolean' ? update.withDelivery : current.withDelivery;
		...
		try {
			...
			await Promise.all(
				subs.map((s) => webpush.sendNotification(s.subscription, JSON.stringify({ title: 'Mise à jour de commande', body: msg, data: { id: saved._id } })).catch(() => null))
			);
		} catch {}
		// FCM push
		try {
			...
			if (tokens.length) {
				await admin.messaging().sendEachForMulticast({ tokens: tokens.map(t => t.token), notification: { title: 'Mise à jour de commande', body: `Statut: ${saved.status}` }, data: { id: String(saved._id) } });
			}
		} catch {}
	});
	...
	router.delete('/:id', requireAdmin, async (req, res) => {
		...
		if (deleted) {
			...
			try {
				...
				await Promise.all(
					subs.map((s) => webpush.sendNotification(s.subscription, JSON.stringify({ title: 'Commande supprimée', body: 'Votre commande a été supprimée.', data: { id: deleted._id } })).catch(() => null))
				);
			} catch {}
			try {
				const admin = initFirebaseAdmin();
				...
				if (tokens.length) {
					await admin.messaging().sendEachForMulticast({ tokens: tokens.map(t => t.token), notification: { title: 'Commande supprimée', body: 'Votre commande a été supprimée.' }, data: { id: String(deleted._id) } });
				}
			} catch {}
		}
	});
